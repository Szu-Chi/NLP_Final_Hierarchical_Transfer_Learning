...
def make_model(cat_num, embedding_matrix, num_tokens, embedding_dim, metrics=METRICS, num_filters = 128, filter_sizes = [3, 4, 5]):
    #num_filters = 128
    #filter_sizes = [3, 4, 5]
    ...
    for filter_size in filter_sizes:
        x = keras.layers.Conv1D(num_filters, filter_size, activation='relu')(embedded_sequences)
        x = keras.layers.MaxPool1D(int(x.shape[1]))(x)
        pooled_outputs.append(x)
    merged = keras.layers.concatenate(pooled_outputs)

...
def model_fit(model, x, y, val_data=None, c_weight=None):
    ...
    if (model.layers[-1].output_shape[1] == 1 and c_weight != None): # if model output is binary
        c_weight = {0: 1, 1:c_weight}
    
    if val_data != None:
        history = model.fit(x, y, batch_size=128, epochs=100, callbacks=[early_stopping],
                        validation_data=val_data, class_weight=c_weight)
    else:
        history = model.fit(x, y, batch_size=128, epochs=100, callbacks=[early_stopping],
                        validation_split=0.15, class_weight=c_weight)
    return 